...
export function ListAccountsView() {
  ...
  return <Box>
    <Box marginBottom="5">
      ...
      { accounts && <>
        { accounts.map(a => <Box key={a.id}>
            <Text variant="normal" color="text100">{accountToName(a)}</Text>
            <Text variant="normal" color="text100">
              <a style={{
                marginLeft: '7px',
                textDecoration: 'underline',
                cursor: 'pointer'
              }} onClick={() => removeAccount(a.id)}>x</a>
            </Text>
        </Box>)}
      </>}
      {loading && <Spinner/>}
    ...
    {error && <Text variant="normal" color="text100" fontWeight="bold">
